...
const signup = asyncHandler(async (req, res) => {
  const { fullName, email, username, password, conformpassword, gender } =
    req.body;

  if (password !== conformpassword) {
    throw new ApiError(400, "password and conform password does not match");
  }
  if (
  ...
  const profilePicMale = `https://avatar.iran.liara.run/public/boy`;
  const profilePicFemale = `https://avatar.iran.liara.run/public/girl`;
